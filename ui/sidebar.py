    ...
    def render(self):
        ...
        if not notes:
            ctk.CTkLabel(
                ...
                text_color="#a688c5", font=ctk.CTkFont(size=12)
            ).pack(pady=40)
            return
        ...
        for i, note in enumerate(notes):
            ...
            if i == self.active_note:
                frame.configure(fg_color=PURPLE_SELECT,
                                border_width=2, border_color=PURPLE_BRIGHT)
            ...
            sub = ctk.CTkLabel(
                frame, text=preview or " ",
                font=ctk.CTkFont(size=11),
                text_color="#a688c5", anchor="w"
            )
            ...
            ctk.CTkLabel(
                bottom, text=note.get("date", ""),
                font=ctk.CTkFont(size=10), text_color="#7a5c94"
            ).pack(side="left")

            ctk.CTkButton(
                bottom, text="🗑", width=28, height=22,
                fg_color="transparent", hover_color="#8B0000",
                font=ctk.CTkFont(size=12),
                command=lambda idx=i: self.on_delete(idx)
            ).pack(side="right")
